...
def make_event(appt):
	...
	try:
		...
		logging.error('ERROR WHEN DUMPING INFO TO EVENT CONFIG FOR FOLLOWING APPOINTMENT\n%s\nERROR FOLLOWS\n%s',
		              str(appt),e)
...
logging.basicConfig(filename='create_event.log',format='[%(asctime)s]%(levelname)s:%(message)s',level=logging.DEBUG)
logging.getLogger('googleapicliet.discovery_cache').setLevel(logging.ERROR)
testl=[{'name': 'Justin Nordeman', 'start': datetime.datetime(2019, 3, 26, 9, 30), 'end': datetime.datetime(2019, 3, 26, 10, 30), 'course': 'Calculus II'}]
make_events(testl)